...
DATA_PATH = os.path.join(BASE_DIR, "cleaned_data_final.pkl")
CHART_DIR = os.path.join(BASE_DIR, "charts")
LAYOUT_PATH = os.path.join(BASE_DIR, "dashboard_layout.py")
...
def test_cleaned_data_load():
    ...
    assert os.path.exists(DATA_PATH), "Missing cleaned dataset."
...
def _chart_files_for_parametrize():
    if not os.path.isdir(CHART_DIR):
        return []
    return [f for f in os.listdir(CHART_DIR) if f.endswith(".py")]


@pytest.mark.parametrize("chart_file", _chart_files_for_parametrize())
...
def test_layout_file_exists():
    ...
    assert os.path.exists(LAYOUT_PATH), "Missing dashboard_layout.py file."
...
def test_layout_execution():
    ...
    for fname in os.listdir(CHART_DIR):
        ...
        if fname.endswith(".py"):
            ...
            with open(os.path.join(CHART_DIR, fname), encoding="utf-8") as fchart:
                code = fchart.read()
